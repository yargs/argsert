...
export default function argsert(typeConfig, ...args) {
  if (
    ...
    (!isRequired(typeConfig) && !isOptional(typeConfig))
  ) {
  ...
  if (args.length < numRequired) {
    throw new Error(
      `Not enough arguments provided. Expected ${numRequired} but received ${
        args.length
      }.`
    );
  }
  ...
  args.forEach((arg, index) => {
    const observedType = Array.isArray(arg)
      ? 'array'
      : arg === null
        ? 'null'
    ...
    const errorMessage = invalidArgMessage.bind(
      this,
      positionName(index),
      typesAtIndex,
      observedType
    ...
    if (
      REQUIRED in typesAtIndex &&
      isMissingFrom(typesAtIndex.required, observedType)
    ) {
      throw new TypeError(errorMessage(REQUIRED));
    }

    if (
      OPTIONAL in typesAtIndex &&
      arg !== undefined &&
      isMissingFrom(typesAtIndex.optional, observedType)
    ) {
...
function getTypes(typeConfig) {
  return typeConfig.split(' ').reduce((result, str, index) => {
    ...
    throw new Error(
      `Invalid type config in the ${positionName(index)} position.`
    );
  }, {});
}
...
function invalidArgMessage(position, types, observed, kind) {
  return `Invalid ${position} argument. Expected ${expectedTypes(
    types,
    kind
...
function expectedTypes(types, kind) {
  return types[kind]
    .concat(
      kind === OPTIONAL && types[OPTIONAL].indexOf('undefined') < 0
        ? 'undefined'
        : []
    )
    .join(' or ');
